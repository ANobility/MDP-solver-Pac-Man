...
class Grid:
    ...
    def __eq__(self, other):
        ...
        for i in range(self.height):
            for j in range(self.width):
                if self.grid[i][j] != other.grid[i][j]: return False
...
class MapAgent(Agent):
    ...
    def registerInitialState(self, state):
        ...
        # map base codes
        print self.reward_map.getWidth()
    ...
    def initialRewardMap(self, state):
        ...
        ghosts = map(lambda x: ((int(x[0][0]), int(x[0][1])), x[1]), api.ghostStatesWithTimes(state))
        ...
        walkDistanceMapOfGhosts = self.getGhostWalkDistance(ghosts[0][0], self.ghosts_facing[0])


        if len(ghosts)>1:
            walkDistanceMapOfGhosts = walkDistanceMapOfGhosts.minimumValueOnMap(
                self.getGhostWalkDistance(ghosts[1][0], self.ghosts_facing[1]))
            ...
            for i in range(1, self.reward_map.getWidth() - 1):
                for j in range(1, self.reward_map.getHeight() - 1):
                    if self.reward_map.getValue(i, j) != '%':
                        distance_ghost=float(walkDistanceMapOfGhosts.getValue(i,j))
    ...
    def getMaxUtility(self, x, y,a_map):
    ...
    def getGhostWalkDistance(self, origin, facing):
        ...
        while queue:
            ...
            while queue:
                ...
                #         distance_map.setValue(next_point[0], next_point[1],
                #                               min(distance, distance_map.getValue(next_point[0], next_point[1])))
